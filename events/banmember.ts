...
module.exports = {
	...
	async execute(
		...
		if (
			newMember.roles.cache.has(bannedrole) &&
			!newMember.roles.cache.has(mainrole)
		) {
			const embed = new MessageEmbed()
				...
				.setDescription(
					"Un-registered or un-accepted member of the MEXT Scholarship Secondary Screening are not permitted in the server. If you think this is a mistake, please contact Sorata#3058"
				)
			...
			return newMember
				.kick(
					"Not a member accepted into secondary screening by the Japanese Government"
				)
				.catch((err) => {
					console.log(err);
				});
