...
module.exports = {
  ...
  async execute(message:Message, args:string[] | string, client:ClientExtensionInterface) {
    ...
    await client.ClientDatabase.setAsync(`prefix:${message.guild?.id}`, args[0])
    const verifyprefix = await client.ClientFunction.getprefix(client, message.guild?.id)
    if(args[0] == verifyprefix){
      return message.reply("I have set your prefix to " + "`" + verifyprefix + "`!!!")
    }else{
      return message.reply("It seems like there has been a problem setting the prefix for this server. Please try again or contact an admin!")
    }
  }
}